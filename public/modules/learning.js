...
(function () {
  ...
  if (!document.getElementById('fk-learning-css')) {
    ...
    .lms .seg button{border:0;background:transparent;font-family:inherit;font-size:15px;font-weight:600;color:var(--muted);padding:8px 15px;border-radius:8px;cursor:pointer}
    ...
    .lms .hero .ey{position:relative;text-transform:uppercase;letter-spacing:.16em;font-size:14px;font-weight:700;color:var(--orange2)}
    ...
    .lms .cbadge{flex:0 0 auto;padding:6px 12px;border-radius:9px;font-size:13.5px;font-weight:800;color:#fff;letter-spacing:.03em}
    ...
    .lms .chip{font-size:14px;font-weight:700;padding:5px 12px;border-radius:999px}.lms .chip.cur{background:#F7E7D6;color:#9A4A18}.lms .chip.lock{background:var(--chip);color:var(--soft)}.lms .chip.ok{background:var(--green-bg);color:var(--green)}.lms .chip.amber{background:var(--amber-bg);color:var(--amber)}
    ...
    .lms .ey2{text-transform:uppercase;letter-spacing:.1em;font-size:13.5px;font-weight:700;color:var(--soft)}
    ...
    .lms .lesson p.ref{text-transform:uppercase;letter-spacing:.09em;font-size:13.5px;font-weight:700;color:var(--soft);margin:0 0 18px}
    .lms .lesson table.sop{width:100%;border-collapse:collapse;margin:14px 0 18px;font-size:15.5px}
    .lms .lesson table.sop th{text-align:left;padding:12px 16px;background:#FAF5ED;font-size:13.5px;text-transform:uppercase;letter-spacing:.05em;color:var(--muted);font-weight:700;border-bottom:1px solid var(--line)}
    ...
    .lms .lms-order .ob{background:#0F1620;padding:9px 16px;font-family:ui-monospace,monospace;font-size:14px;color:#9FB0C3;border-bottom:1px solid var(--grid)}.lms .lms-order .ob .dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--orange);margin-right:7px}.lms .lms-order .ob b{color:#E7ECF2}
    .lms .lms-order table{width:100%;border-collapse:collapse}.lms .lms-order td{padding:9px 16px;border-bottom:1px solid var(--grid);vertical-align:top}.lms .lms-order td.k{color:#8FA1B5;width:112px;font-size:14px;text-transform:uppercase}.lms .lms-order td.v{font-family:ui-monospace,monospace}.lms .lms-order tr:last-child td{border-bottom:0}.lms .lms-order .sel{background:#3a2a1a;border:1px solid #6b4a22;border-radius:6px;padding:2px 8px;color:#F0C998}
    .lms .gate2{margin-top:28px;border-top:1px solid var(--line);padding-top:18px}.lms .meter{font-size:14px;font-weight:700;color:var(--muted);background:#FBF6EE;border:1px solid var(--line);padding:6px 12px;border-radius:999px;float:right}
    .lms .q{background:#FBF6EE;border:1px solid var(--line);border-radius:16px;padding:20px 24px;margin-top:16px}.lms .q .tag{font-size:13.5px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#9A4A18}.lms .q .tag.ft{color:var(--ai)}.lms .q .tag.apt{color:#185FA5}
    .lms .kbtop{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:6px}.lms .back.pdf{color:#9A4A18}
    .lms .mt .acc{display:flex;flex-direction:column;line-height:1.15}.lms .mt .acc b{font-size:15px;color:var(--ink)}.lms .mt .acc span{font-size:13.5px;color:#8B8173}.lms .mt .acc .muted{color:#B8AE9F;font-weight:400}
    ...
    .lms .ff{display:flex;justify-content:space-between;align-items:center;margin-top:26px;gap:14px;flex-wrap:wrap}.lms .ff .h{color:var(--muted);font-size:15px}
  ...
  // ---------- trainee: course list ----------
  function renderList(courses, extra) {
    if (!Array.isArray(courses)) courses = [courses];
    extra = extra || [];
    myCourses = courses;
    const assignedCards = courses.map(function (c) {
    ...
    const availCards = extra.map(function (c) {
      return '<div class="card"><span class="cbadge" style="background:#C2562E">' + String(c.department || '').toUpperCase() + '</span>' +
        '<div><div class="ttl">' + c.title + '</div><div class="sub">Start the course</div></div>' +
        '<div class="right"><button class="btn lmsStart" data-slug="' + (c.slug || '') + '">Start</button></div></div>';
    }).join('');
    let html = seg() +
    ...
    if (courses.length) {
      ...
      html += '<div class="sech"><h2>Assigned to you</h2><span class="n">' + (nDone === courses.length ? 'All complete' : (courses.length - nDone) + ' in progress') + '</span></div>' + assignedCards +
    ...
    if (extra.length) {
      html += '<div class="sech"><h2>Available</h2></div>' + availCards;
    }
    if (!courses.length && !extra.length) {
      html += '<div class="card" style="opacity:.75"><div><div class="ttl">No courses have been added to your department yet</div><div class="sub">Coming soon \u2014 new courses will appear here when they\u2019re added for your team.</div></div></div>';
    }
    lms().innerHTML = html;
    ...
    Array.from(document.querySelectorAll('.lmsStart')).forEach(function (b) {
  ...
  async function openSession(sid) {
    ...
    lms().innerHTML = '<button class="back" id="bk">\u2039 Back to course</button>' +
      ...
      '<p style="color:#8B8173;font-size:14.5px;margin:0">Pass all of these to unlock the next session. The obvious answer is often wrong \u2014 you can retry.</p>' +
      checks +
  ...
  async function grade(btn, sid, totalChecks) {
    ...
    if (out.result === 'pass') {
      ...
      fb.innerHTML = (out.feedback || 'Not quite.') + (out.cost ? '<div class="c">Cost: ' + out.cost + '</div>' : '') + '<div style="margin-top:5px;color:#8B8173;font-size:14px">Try again.</div>';
    }
  }
  ...
  function openKB(it) {
    ...
    if (it.type === 'rate_card' && cfg) {
      ...
      body += '</table><p style="color:#8B8173;font-size:14px;margin-top:8px">Verified ' + (it.verified_on || '') + '.</p>';
    }
  ...
  function printDoc(title, verified, bodyHtml) {
    ...
    var css = 'body{font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#241F1B;max-width:760px;margin:32px auto;padding:0 28px;line-height:1.6}'
      + 'h1{font-size:23px;margin:0 0 4px}h4{font-size:14px;margin:18px 0 6px}p,li{font-size:15px}'
      + 'table{border-collapse:collapse;width:100%;margin:12px 0}th,td{border:1px solid #E9E2D6;padding:7px 10px;text-align:left;font-size:14px;vertical-align:top}th{background:#F4EFE7}'
      + '.warn{background:#F6E9E6;border-left:3px solid #B0453A;padding:10px 13px;margin:14px 0;border-radius:6px;font-size:14.5px}'
      + '.fc{display:none}.foot{color:#8B8173;font-size:13.5px;margin-top:26px;border-top:1px solid #E9E2D6;padding-top:10px}';
  ...
  async function bootInner(view) {
    ...
    const assignedIds = {};
    courses.forEach(function (c) { assignedIds[c.id] = true; });
    const extra = avail.filter(function (c) { return !assignedIds[c.id]; });
    renderList(courses, extra);
    wireSeg();
  }
